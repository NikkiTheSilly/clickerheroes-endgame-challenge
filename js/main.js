...
function calculateProgression() {
  ...
  dogcogEffect = 0.9999999999 * (1 - Math.pow(Math.E, -0.01 * dogcog));

  var ponyBonus =
    pony > 100 ? Math.log10(pony) * 2 + 1 : Math.log10(pony * pony * 10 + 1);
  ...
  for (i = 0; i < 250; i++) {
    ...
    let gilds = Math.max(
      1,
      Math.floor((lghsStart - ponyBonus) / Math.log10(1 + tp) / 10 - 10)
    );
    ...
    if (ROOT2) {
      ...
      let nog =
        (lghsStart + Math.log10(1 / 0.95) * chor + hsSplit) / 2.5 +
        (Math.log10(2.5) * 2) / 5;
    ...
    var goldBonus =
      zone > zoneTL // Autoclickers or Xyliqil gold increase
        ? Math.min(306, cps)
        : ROOT2
        ? 0
        : xylBonus;
    ...
    hlevel =
      (zone * Math.log10(GOLD_SCALE) +
        1.5 * effectivelghs +
        hsGoldAdjust +
        goldBonus140 -
        costReduction(getHeroAttr(hnum, "lv1cost")) +
        goldBonus -
        Math.log10(15)) /
      Math.log10(getHeroAttr(hnum, "costScale"));
    lghsEnd =
      (zone / 5 - 20) * Math.log10(1 + tp) + Math.log10((20 * (1 + tp)) / tp);
    lghsEnd += ponyBonus;
    if (ANCIENT_SOULS >= 21000 && !ROOT2)
      lghsEnd -= zone > 1e6 ? Math.log10(400) : Math.log10(20);
    lghsChange =
      lghsEnd - lghsStart > 50
        ? lghsEnd - lghsStart
    ...
    if (zone >= MAX_ZONE) {
      // Stop if encountering infinite ascension
      break;
    }
...
function formatTime(durationSeconds) {
  ...
  if (hours < 72) {
    ...
    hours = dl / 3600;
    return (
      (years > 0 ? years.toLocaleString() + "y " : "") +
      days +
      "d " +
      hours.toFixed(2) +
      "h"
...
function zoneReached(lgHS, i, active = true) {
  let R =
    Math.log10(getHeroAttr(i, "damageScale")) /
    Math.log10(getHeroAttr(i, "costScale")) /
    25;
  let lgDmgMultPerZone =
    Math.log10(GOLD_SCALE) * R +
    (ROOT2 * (ANCIENT_SOULS >= 6400) * Math.log10(1.01)) / 10;
  let efficiency =
    getHeroAttr(i, "dps") -
    R *
      (costReduction(getHeroAttr(i, "lv1cost")) +
        175 * Math.log10(getHeroAttr(i, "costScale")));
  ...
  let RHS = efficiency + (2.4 + active * 0.5) * lgHS - 2 + startingGold * R; // Minus 2 to account for boss HP
  RHS += active ? hsActiveDmgAdjust : hsIdleDmgAdjust;
  ...
  let reqzone =
    (heroUpgradeBaseCost(i) - startingGold) / Math.log10(GOLD_SCALE);
  ...
  for (j = 0; j < nbp - 1; j++) {
    ...
    let lghp1 =
      lghp0 + (HP_SCALE[0][j + 1] - HP_SCALE[0][j]) * Math.log10(hpscale);
    let z0 = HP_SCALE[0][j];
    let z1 = HP_SCALE[0][j + 1];

    if (
      z1 >= reqzone &&
      lghp0 - lgDmgMultPerZone * z0 <= RHS &&
      lghp1 - lgDmgMultPerZone * z1 > RHS
    ) {
  ...
  if (M < 0) {
    // Infinite ascension (monster scaling < damage increase)
    return MAX_ZONE;
  }
  let zone =
    M * (RHS - lghp0 + Math.log10(HP_SCALE[1][nbp - 1]) * HP_SCALE[0][nbp - 1]);
  return zone;
}
...
function heroUpgradeBaseCost(hnum) {
  ...
  let costScale = getHeroAttr(
    hnum - (ROOT2 && hnum == HEROES.length - 1),
    "costScale"
  );
  return (
    costReduction(getHeroAttr(hnum, "lv1cost")) + Math.log10(costScale) * level
  );
}
...
function dataArrayToHTML(data) {
  ...
  for (i = 0; i < data.length; i++) {
    tdclass = classes[i] || "";
    data2.push(
      "<td class=" + tdclass + ">" + data[i].join("</td><td>") + "</td>"
    );
  }
